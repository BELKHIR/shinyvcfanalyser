...
from moments import Misc, Numerics, local_scipy_dual_anneal #This is an adapted version of scipy.optimize.dual_annealing debuged to allow passing bounds parameter to bounded minimizer functions 
...
#@Khalid Belkhir 2020: 1) count_sup/count_inf: the out-of-bound issue is solved. 2) fmin/xmin: the logL and optim. param returned are guaranteed to be the maxlogL.
_counter = 0
#_count_sup = 0
#_count_inf = 0
_fmin = 999999999
...
def _object_func(params, data, model_func, 
    ...
    global _fmin
    global _xmin
    if -result/ll_scale < _fmin:
        _fmin = -result/ll_scale
        _xmin = params_up

    if (verbose > 0) and (_counter % verbose == 0):
        param_str = 'array([%s])' % (', '.join(['%- 12g'%v for v in params_up]))
        output_stream.write('%-8i, %-12g, %-12g, %s%s' % (_counter, _fmin * -ll_scale, result, param_str,
                                                   os.linesep))
...
def optimize_log(p0, data, model_func, lower_bound=None, upper_bound=None,
    ...
    #@Chris Fraïsse 2020: project the true best parameters up
    xmin = _project_params_up(_xmin, fixed_params)
    ...
    #@Chris Fraïsse 2020: return the true parameter array
        return xmin
...
def optimize_anneal(p0, data, model_func, lower_bound=None, upper_bound=None,
                  verbose=0, flush_delay=0.5, 
    ...
    """
    @Christelle Fraïsse 2020
    Configuration: scipy <= 0.14 ; numpy 1.17.0
    https://docs.scipy.org/doc/scipy-0.15.1/reference/generated/scipy.optimize.anneal.html
    """                  
    if output_file:
        output_stream = open(output_file, 'w')
    else:
    ...
    #@Khalid Belkhir 2020: as p0 is log-transformed before calling the function annealing, we need to do the same with the bound values
    bounds=list(zip(numpy.log(lower_bound), numpy.log(upper_bound))) 

    outputs = scipy.optimize.anneal(_object_func_log, 
                                       numpy.log(p0),
                                       args = args, schedule=schedule,
                                       full_output=full_output, T0=Tini, Tf=Tfin,
                                       maxiter=maxiter, learn_rate=learn_rate, lower=bounds[0], upper=bounds[1])
    #@Chris Fraïsse 2020: extract and name the outputs of the optimize function
    xopt, fopt, Tend, feval, iters, accept, status = outputs
    #project the best parameters up
    xopt = _project_params_up(numpy.exp(xopt), fixed_params)
    #@Chris Fraïsse 2020: project the true best parameters up
    xmin = _project_params_up(_xmin, fixed_params)
...
def optimize_dual_anneal(p0, data, model_func, lower_bound=None, upper_bound=None,
                 verbose=0, flush_delay=0.5, accept=None, visit=None,
                 multinom=True, maxiter=None, full_output=False,
                 func_args=[], func_kwargs={}, fixed_params=None, ll_scale=1,
                 output_file=None, Tini=None, no_local_search=True, local_method = None, local_maxiter = None):
    """
    @Khalid Belkhir 2020
    https://docs.scipy.org/doc/scipy/reference/generated/scipy.optimize.dual_annealing.html

    https://stats.stackexchange.com/questions/436052/simulated-annealing-vs-basin-hopping-algorithm
    The reason for Simulated Annealing to be Deprecated is not because Basin-hopping outperform it theoretically.
    It is because the specific implementation done for Simulated Annealing in the library is a special case of the second.
    If you want to use a Simulated Annealing algorithm I recommend you to use scipy.optimize.dual_annealing instead, but with visit=qv=1, accept=qa=1
    (this recover Classical Simulated Annealing, i.e. the temperature decreases logarithmically).
    Other parameter selection leads to more sophisticated Annealing processes, like visit=qv=2, accept=qa=1
    (which recover the Fast Simulated Annealing, i.e. the temperature decrease up to inverse).
    """                 
    if output_file:
        output_stream = open(output_file, 'w')
    else:
    ...
    #as p0 is log-transformed before calling the function dual_annealing, we need to do the same with the bound values
    bounds=list(zip(numpy.log(lower_bound), numpy.log(upper_bound))) 
    ...
    if local_method != None:
        local_search_options={"method":local_method, "options":{"maxiter": local_maxiter } }

    #Here we will use a local version of scipy.optimize.dual_annealing debuged to allow passing bounds parameter to bounded minimizer functions 
    #outputs = scipy.optimize.dual_annealing(_object_func_log,
    outputs = local_scipy_dual_anneal.dual_annealing(_object_func_log,
                                            bounds=bounds,
                                            x0=numpy.log(p0),
                                            args = args, accept=accept, visit=visit,
                                            initial_temp=Tini, local_search_options= local_search_options,
                                            maxiter=maxiter, no_local_search = no_local_search)
    xopt = outputs.x
    xopt = _project_params_up(numpy.exp(xopt), fixed_params)
    #project the true best parameters up
    xmin = _project_params_up(_xmin, fixed_params)
    ...
    if not full_output:
        return xopt
    #append to the dict. and return the true logL (fmax) and parameter array (xmin)
    else:
        outputs.update({"fmax":_fmin, "xmax":xmin})
        return outputs
