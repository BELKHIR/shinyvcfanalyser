...
class LocalSearchWrapper(object):
    ...
    def __init__(self, bounds, func_wrapper, **kwargs):
        ...
        if not self.kwargs:
            ...
            # si l'on veut utiliser une méthode avec bounds (L-BFGS-B, TNC, SLSQP, trust-constr )
            # comme on ne peut pas passer bounds= dans **kwargs qui vient de **local_search_options !!!
            # on aurait sinon deux fois le pram. bounds dans __init__
            # kwargs sera passé à self.minimizer(self.func_wrapper.fun, x, **self.kwargs)
