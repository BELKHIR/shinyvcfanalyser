...
def make_data_dict_vcf_Mem(
    vcf_filename, popinfo_filename, filter=True, maxMemPercent =80, flanking_info=[None, None]
):
    """
    Parse a VCF file containing genomic sequence information, along with a file
    identifying the population of each sample, and store the information in 
    ...
    for line in vcf_file:
        ...
        if not full_info:
            continue
        snp_dict["calls"] = calls_dict
        #If we reach 80 percent of availbale memory we stop reading snp
        if psutil.virtual_memory().percent < maxMemPercent:
            data_dict[snp_id] = snp_dict
            nbsnp += 1
        else:
            outofmem = True
    ...
    vcf_file.close()
    return (outofmem, nbsnp, data_dict)
